...
inline constexpr std::array<std::array<uint64_t, 64>, 2> kPawnAttack = []() {
  ...
  std::array<int, 2> rank_offset{+1, -1};

  
  for (int square = 0; square < 64; ++square) {
    for (uint8_t color = 0; color < 2;
      ++color) { // weiße Bauern gehen nach oben, schwarze nach unten
      int rank = square / 8;
      rank += rank_offset[color];
      if (rank > 7 ||
          rank < 0) {
        continue;
      }
      for (uint8_t file_direction = 0; file_direction < 2; ++file_direction) {
        int file = square % 8;
        file += file_offset[file_direction];
        if (file >= 0 && file < 8) {
          int target = rank * 8 + file;
          attack[color][static_cast<uint64_t>(square)] |=
              square_bb(static_cast<Square>(
                  target)); // wir nutzen die 0 als White und 1 als Black (enum
                            // class ist hier nicht nötig)
        }
      }
    }
  }
...
static_assert(kPawnAttack[std::to_underlying(Color::White)]
                         [std::to_underlying(Square::e4)] ==
              (square_bb(Square::d5) | square_bb(Square::f5)));

static_assert(kPawnAttack[std::to_underlying(Color::White)]
                         [std::to_underlying(Square::h7)] ==
              (square_bb(Square::g8)));

static_assert(kPawnAttack[std::to_underlying(Color::White)]
                         [std::to_underlying(Square::a2)] ==
              (square_bb(Square::b3)));

static_assert(kPawnAttack[std::to_underlying(Color::Black)]
                         [std::to_underlying(Square::e4)] ==
              (square_bb(Square::d3) | square_bb(Square::f3)));
...
inline constexpr std::array<uint64_t, 64> kKnightAttack = []() {
  ...
  for (uint8_t square = 0; square < 64; ++square) {
    // uint8_t: bei negative Overflow wird die Zahl > 7, wichtig für unseren
    // Check später
...
inline constexpr std::array<uint64_t, 64> kKingAttack = []() {
  ...
  for (int square = 0; square < 64; ++square) {
    ...
    for (uint8_t offset = 0; offset < 8; ++offset) {
      ...
      if (new_rank <= 7 && new_rank >= 0 && new_file <= 7 && new_file >= 0) {
        table[static_cast<std::size_t>(square)] |=
            square_bb(static_cast<Square>((new_rank * 8) + new_file));
      }
    }
  }
  return table;
...
static_assert(kKingAttack[std::to_underlying(Square::a1)] ==
              (square_bb(Square::a2) | square_bb(Square::b2) |
               square_bb(Square::b1)));

static_assert(kKingAttack[std::to_underlying(Square::h8)] ==
              (square_bb(Square::g7) | square_bb(Square::g8) |
               square_bb(Square::h7)));

static_assert(kKingAttack[std::to_underlying(Square::e4)] ==
              (square_bb(Square::d3) | square_bb(Square::e3) |
               square_bb(Square::f3) | square_bb(Square::d4) |
               square_bb(Square::f4) | square_bb(Square::d5) |
               square_bb(Square::e5) | square_bb(Square::f5)));

// Sliding Pieces:Ergebnis hängt von occupied ab -> Laufzeitfunktion statt Tabelle
